...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < frames; i++) { left[i] = audio[i * 2], right[i] = audio[i * 2 + 1]; }
    free(audio);
    ...
    if (!avg_mags_left || !avg_mags_right) { return 1; }

    float *frame_mags = malloc(sizeof(float) * bins);
    if (!frame_mags) { return 1; }

    float total_weight_left = 0.0f, total_weight_right = 0.0f;

    for (int f = 0; f < num_frames; f++) {
        if (f % 100 == 0) { printf("Processing frame %d / %d\r", f, num_frames); }
    ...
    if (!left_out || !right_out) { return 1; }
    ...
    for (int i = 0; i < out_frames; i++) {
        if (fabsf(left_out[i]) > peak) { peak = fabsf(left_out[i]); }
        if (fabsf(right_out[i]) > peak) { peak = fabsf(right_out[i]); }
    }
    if (peak > 0.0f) {
        float scale = 0.9f / peak;
        for (int i = 0; i < out_frames; i++) { left_out[i] *= scale, right_out[i] *= scale; }
    }
    ...
    for (int i = 0; i < out_frames; i++) { interleaved[i * 2] = left_out[i], interleaved[i * 2 + 1] = right_out[i]; }
